...
public:
    ...
    //EL ERROR QUE ME DABA ERA DEVOLVER UNA VARIABLE LOCAL T ELEM EN LUGAR DEL PUNTERO->ELEM
    //PARA LA PROXIMA TENER EN CUENTA QUE ES MEJOR HACER THROWS PARA HACER MAS LEGIBLE EL CODIGO
    //Y LOS TRY CATCH EN LA MAIN
    T const& kesimo(int k) const {

        Link punt_aux = raiz ;
        if (raiz == nullptr || k < 1 || k > nelems)
            throw std::out_of_range("k fuera de rango");


            while (punt_aux!=nullptr) {
                if (k > punt_aux->tam_i) {
                    k -= punt_aux->tam_i;
                    punt_aux = punt_aux->dr;
                }
                else if (k < punt_aux->tam_i) {
                    punt_aux = punt_aux->iz;
                }
                else {
                    return punt_aux->elem;
                }

            }

            throw std::runtime_error("Árbol inconsistente");
    }
...
protected:
    ...
    bool inserta(T const& e, Link& a) {
        ...
        if (a == nullptr) { // se inserta el nuevo elemento e
            ...
            //SOLO SUMAR 1 SI EL ARBOL CRECE 
            if (crece) {
                a->tam_i += 1;
                reequilibraDer(a);
            }
    ...
    r1= su hijo izquierdo sigue siendo A r1.tam_i_viejo = r1.tam_i_nuevo;
    en cambio,
    r2= si hijo izquierdo = r2.tam_i_viejo - r1.tam_i
    */
    ...
    /*
          ...
          r1.tam_i_viejo = r1.tam_i_nuevo
          r2_tam_i_nuevo = r2_tam_viejo + r1_tam_i
    */
